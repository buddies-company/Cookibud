    ...
    return (
        {
            ...
            "read_items_sorted_by_expiration": ReadFridgeItemsSortedByExpirationUseCase(
                repo
            ),
            "search_items": SearchFridgeItemsUseCase(repo),
        },
        token.user_id,
    )
...
def read_fridge_sorted_by_expiration(
    usecases_and_user: tuple = Depends(get_fridge_usecases), sort_dir: str = "asc"
):
...
@router.get("/search")
def search_fridge_items(
    usecases_and_user: tuple = Depends(get_fridge_usecases), query: str = ""
):
    """Search the authenticated user's fridge items by name"""
    usecases, user_id = usecases_and_user
    try:
        items = usecases["search_items"](user_id, query)
        return {"items": items}
    except AccessDeniedError as e:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=str(e)) from e
...
def add_fridge_item(
    item: FridgeItem, usecases_and_user: tuple = Depends(get_fridge_usecases)
):
...
def mark_fridge_item_as_used(
    item_id: str, usecases_and_user: tuple = Depends(get_fridge_usecases)
):
